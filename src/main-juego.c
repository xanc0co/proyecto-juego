...
int main(int argc, char **argv) 
{
    ...
    jugador.tamaño = (anchoTile < altoTile) ? anchoTile + 2 : altoTile + 2;
    jugador.tamaño = (jugador.tamaño < 10) ? 10 : jugador.tamaño;
    jugador.velocidadX = 0;
    jugador.velocidadY = 0;
...
void cargarMapaDesdeArchivo(const char* nombreArchivo, char mapa[MAP_HEIGHT][MAP_WIDTH]) {
    ...
    while (fila < MAP_HEIGHT && fgets(linea, sizeof(linea), archivo) != NULL) {
        ...
        for (int col = 0; col < MAP_WIDTH && col < largo; col++) {
            mapa[fila][col] = linea[col];
        }

        for (int col = largo; col < MAP_WIDTH; col++) {
            mapa[fila][col] = 'f';
        }
...
void resolverColision(Jugador* jugador, Rectangulo* rectangulo) {
    ...
    /* Si el jugador viene cayendo, se apoya sobre la plataforma */
    if (jugador->velocidadY >= 0 && playerBottom >= rectTop && playerBottom <= rectBottom) {
    ...
    /* Si viene subiendo, choca desde abajo */
    if (jugador->velocidadY < 0 && playerTop <= rectBottom && playerTop >= rectTop) {
    ...
    if (playerRight > rectLeft && playerLeft < rectRight) {
        if (playerRight - rectLeft < rectRight - playerLeft) {
